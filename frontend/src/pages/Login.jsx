...
const BG_IMAGES = [
  'https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=1920&q=85&fit=crop',
...
const INPUT = "w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 transition-all";
...
export default function Login() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setFading(true);
      setTimeout(() => { setCurrent(p => (p + 1) % BG_IMAGES.length); setFading(false); }, 800);
    }, 4000);
    return () => clearInterval(timer);
  }, []);
 
  useEffect(() => { BG_IMAGES.forEach(src => { const img = new Image(); img.src = src; }); }, []);
  ...
  async function sendForgotOTP() {
    ...
    try {
      ...
      if (res.otp) setFpDevOTP(res.otp);
      setFpStep(2);
  ...
  function openForgot() { setForgotOpen(true); setFpStep(1); setFpDevOTP(''); }
    ...
    <div className="min-h-screen flex overflow-hidden font-sans">
      ...
      <div className="hidden lg:flex lg:flex-1 relative overflow-hidden">
        <div className="absolute inset-0 bg-cover bg-center transition-all duration-700"
          style={{ backgroundImage: `url(${BG_IMAGES[current]})`, opacity: fading ? 0 : 1 }} />
          ...
          <div className={`transition-all duration-700 ${fading ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'}`}>
            <div className="mb-6">
              <div className="flex gap-1.5 mb-8">
                {BG_IMAGES.map((_, i) => (
                  <div key={i} className={`h-0.5 rounded-full transition-all duration-500 ${i === current ? 'w-8 bg-white' : 'w-2 bg-white/30'}`} />
                ))}
              </div>
              <h2 className="text-4xl font-bold text-white leading-tight mb-3">{CAPTIONS[current].title}</h2>
              <p className="text-white/70 text-lg">{CAPTIONS[current].sub}</p>
            </div>
            <div className="flex gap-6">
              {[{ value:'4', label:'Standards' }, { value:'100%', label:'Compliant' }, { value:'∞', label:'Clients' }].map((s, i) => (
      ...
      <div className="w-full lg:w-[480px] flex-shrink-0 flex flex-col justify-center px-8 md:px-12 bg-gradient-to-br from-slate-50 to-blue-50/40 relative">
        ...
        <div className="relative z-10 max-w-sm mx-auto w-full">
          ...
          <div className="space-y-3 mb-6">
            {[
              { name:'Google', icon: <svg viewBox="0 0 24 24" className="w-4 h-4" fill="none"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg> },
              { name:'Microsoft', icon: <svg viewBox="0 0 24 24" className="w-4 h-4"><path fill="#F25022" d="M1 1h10v10H1z"/><path fill="#00A4EF" d="M13 1h10v10H13z"/><path fill="#7FBA00" d="M1 13h10v10H1z"/><path fill="#FFB900" d="M13 13h10v10H13z"/></svg> },
            ].map(s => (
              <button key={s.name}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="email" value={form.email}
                ...
                placeholder="you@firm.com" className={INPUT} autoComplete="email" autoFocus />
            </div>
          ...
          <p className="text-center text-sm text-slate-500 mt-6">
            New to FinStatement?{' '}
            <a href="/register" className="text-indigo-600 font-semibold hover:text-indigo-700">Create account</a>
          </p>
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm mx-4">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-slate-900">
                {fpStep === 1 ? 'Reset Password' : 'Enter Code & New Password'}
              </h2>
              <button onClick={closeForgot} className="text-slate-400 hover:text-slate-600 text-xl">✕</button>
            </div>
            ...
            {fpStep === 1 ? (
              ...
              <div className="space-y-4">
                <p className="text-sm text-slate-500">Code sent to <strong>{fpEmail}</strong></p>
                {fpDevOTP && (
                ...
                <div>
                  ...
                  <input type="text" maxLength={6} value={fpOTP}
                    ...
                    className={INPUT + ' text-center tracking-widest font-bold text-lg'} placeholder="000000" autoFocus />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">New Password</label>
                  <input type="password" value={fpPass} onChange={e => setFpPass(e.target.value)}
                    className={INPUT} placeholder="At least 8 characters" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">Confirm Password</label>
                  <input type="password" value={fpPass2} onChange={e => setFpPass2(e.target.value)}
