...
// This eliminates duplicates caused by mixed capitalisation.
      ...
      {open && (
        ...
        >
          ...
          {query && !options.find(o => o.toLowerCase() === query.toLowerCase()) && (
            <div
              onClick={() => select(query.charAt(0).toUpperCase() + query.slice(1).toLowerCase())}
            ...
            >
              ...
              Add "{query.charAt(0).toUpperCase() + query.slice(1).toLowerCase()}" as new FS head
            </div>
          )}
...
export default function Mapping() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      // Load master grouping from cache if method hasn't changed — avoids redundant fetch
      const cachedMaster = masterCacheRef.current;
      const masterPromise = (cachedMaster.method === method && cachedMaster.data.length > 0)
        ? Promise.resolve(cachedMaster.data)
        : mappingAPI.master(method).then(d => {
  ...
  async function downloadFSGroupings() {
    try {
      ...
      a.href     = url;
      a.download = `FS-Groupings-${method}.csv`;
      a.click();
      URL.revokeObjectURL(url);
      toast.success('FS Groupings downloaded');
    } catch (err) {
  ...
  async function saveMapping(subGrouping, data) {
    ...
    try {
      ...
      // Revert on failure
      setMappings(prev => ({ ...prev, [subGrouping]: previousState }));
  ...
  async function quickChangeGroup(subGrouping, groupName) {
    ...
    const masterRow = master.find(m => toSentenceCase(m.groupName) === normName);
    const data = {
      subGrouping,
      groupName:        normName,
      subGroupName:     masterRow?.subGroupName || subGrouping,
      noteGroupId:      masterRow?.noteGroupId  || generateNoteGroupId(normName),
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <button onClick={downloadFSGroupings}
            className="px-3 py-2 border border-slate-300 text-slate-700 text-sm rounded-lg hover:bg-slate-50 flex items-center gap-1.5" title="Download master FS Groupings list as CSV">
            ⬇ FS Groupings
          </button>
